<?php 
		...
		$alterar = "UPDATE tb_banners SET status = 'ativo' WHERE id = '$id_status'";
		$alterar_exec = mysql_query($alterar); 
		}
		if($alterar_exec) { $sucesso = "157"; }	else { $erro = "157"; }		
		}
		...
		$item = $_POST['item'];
		foreach($item as $id_status => $item) {
		$alterar = "UPDATE tb_banners SET status = 'inativo' WHERE id = '$id_status'";
		$alterar_exec = mysql_query($alterar); 
		}
		if($alterar_exec) { $sucesso = "157"; }	else { $erro = "157"; }		
		}
		...
		//-------------------- Removendo Banner --------------------------
		if($acao == "remover-banner") {
		$id_remover = $_GET['id'];
		$excluir = "DELETE FROM tb_banners WHERE id = '$id_remover'";
		$excluir_exec = mysql_query($excluir);
		if($excluir_exec) { 
		$sucesso = "155";
		} else { $erro = "155"; }
		}
		//-------------------- Fim do Removendo Banner --------------------
		
		
		//-------------------- Removendo Banners [Em lote] ----------------	
		if($acao == "remover-banners-lote") {
		// Trabalha os itens
		$item = $_POST['item'];
		...
		if($excluir_exec) { $sucesso = "156"; }	else { $erro = "156"; }	
		}
		//-------------------- Fim do Removendo Banner [Em lote] ----------
		...
		// Executa a busca visual e a busca completa dos banners
		$busca_visual = mysql_query("SELECT * FROM tb_banners $where_busca $ordem_busca LIMIT $ini_busca, $fim_busca");
		$busca_completa = mysql_query("SELECT * FROM tb_banners $where_busca");
