<?php 
		...
		if($_GET['id']) { $id_produto = $_GET['id']; }
		else { die("Erro de acesso."); }
		// Id do responsável pelo produto
		$id_responsavel = $_SESSION['NUPILL_ID'];
		$busca_info = mysql_query("SELECT * FROM tb_produtos WHERE id = '$id_produto'");
		$num_info = mysql_num_rows($busca_info);
		
		// Se não há o id
		if ($num_info == 0) {
		die("Esta informa&ccedil;&atilde;o de produto n&atilde;o &eacute; v&aacute;lida.");
		} else {		
