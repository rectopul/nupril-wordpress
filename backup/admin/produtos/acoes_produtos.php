<?php 
		...
		$alterar = "UPDATE tb_produtos SET status = 'ativo' WHERE id = '$id_status'";
		...
		$item = $_POST['item'];
		foreach($item as $id_status => $item) {
		$alterar = "UPDATE tb_produtos SET status = 'inativo' WHERE id = '$id_status'";
		$alterar_exec = mysql_query($alterar); 
		}
		if($alterar_exec) { $sucesso = "57"; }	else { $erro = "57"; }		
		}
		...
		$id_remover = $_GET['id'];
		$excluir = "DELETE FROM tb_produtos WHERE id = '$id_remover'";
		$excluir_exec = mysql_query($excluir);
		if($excluir_exec) { 
		$sucesso = "55";
		} else { $erro = "55"; }
		}
		//-------------------- Fim do Removendo Produto --------------------
		
		
		//-------------------- Removendo Produtos [Em lote] ----------------	
		if($acao == "remover-produtos-lote") {
		// Trabalha os itens
		$item = $_POST['item'];
		...
		//-------------------- Fim do Removendo Produto [Em lote] ----------
		...
		$filtragem = true;
		$where_busca .= " 
		AND (referencia LIKE '%".$p."%'
		OR nome LIKE '%".$p."%'
		OR apelido LIKE '%".$p."%'
		OR nome_marca LIKE '%".$p."%'
		OR nome_categoria LIKE '%".$p."%'
		OR nome_subcategoria LIKE '%".$p."%'
		OR nome_categoria_promocional LIKE '%".$p."%'
		OR nome_pais LIKE '%".$p."%'
		OR descricao LIKE '%".$p."%'
		OR dados_tecnicos LIKE '%".$p."%'
		...
		// Executa a busca visual e a busca completa dos produtos
		$busca_visual = mysql_query("SELECT * FROM tb_produtos $where_busca $ordem_busca LIMIT $ini_busca, $fim_busca");
		$busca_completa = mysql_query("SELECT * FROM tb_produtos $where_busca");
