<?php 
		...
		//-------------------- Removendo Marca e Produtos --------------------------
		if($acao == "remover") {
		$id_remover = $_POST['id'];
		$excluir_produtos = "DELETE FROM tb_produtos WHERE id_marca = '$id_remover'";
		$excluir_produtos_exec = mysql_query($excluir_produtos);
		if($excluir_produtos_exec) { 
		$excluir = "DELETE FROM tb_marcas WHERE id = '$id_remover'";
		...
		//-------------------- Transferindo os produtos e Removendo a Marca --------------------------
		if($acao == "transferir-remover") {
		$id_remover = $_POST['id'];
		$id_marca_destino = $_POST['id_marca_destino'];
		//Busca os dados da marca destino
		$busca_marca_destino = mysql_query("SELECT * FROM tb_marcas WHERE id = '$id_marca_destino'");
		$dados_marca_destino = mysql_fetch_array($busca_marca_destino);
		$nome_marca_destino = $dados_marca_destino['nome'];
		$apelido_marca_destino = $dados_marca_destino['apelido'];
		$alterar = "UPDATE tb_produtos SET id_marca = '$id_marca_destino', nome_marca = '$nome_marca_destino', apelido_marca = '$apelido_marca_destino' WHERE id_marca = '$id_remover'";
		...
		//-------------------- Fim do Transferindo os produtos e Removendo a Marca --------------------
		
		//-------------------- Status = Ativo ---------------	
		if($acao == "ativar-marca") {
		$id_alterar = $_POST['id'];
		$alterar = "UPDATE tb_marcas SET status = 'ativo' WHERE id = '$id_alterar'";
		$alterar_exec = mysql_query($alterar); 
		if($alterar_exec) { $sucesso = "65"; }	else { $erro = "65"; }		
		}
		//-------------------- Fim do Status = Ativo ---------
		
		//-------------------- Status = Inativo ---------------	
		if($acao == "desativar-marca") {
		$id_alterar = $_POST['id'];
		$alterar = "UPDATE tb_marcas SET status = 'inativo' WHERE id = '$id_alterar'";
		$alterar_exec = mysql_query($alterar); 
		if($alterar_exec) { $sucesso = "65"; }	else { $erro = "65"; }		
		}
		...
		if($acao == "ativar-marca-produto") {
		$id_alterar = $_POST['id'];		
		$alterar = "UPDATE tb_marcas SET status = 'ativo' WHERE id = '$id_alterar'";
		$alterar_exec = mysql_query($alterar); 
		if($alterar_exec) { 
		$alterar_produtos = "UPDATE tb_produtos SET status = 'ativo' WHERE id_marca = '$id_alterar'";
		...
		//-------------------- Status = Inativo E Status Produto = Inativo ---------------	
		if($acao == "desativar-marca-produto") {
		$id_alterar = $_POST['id'];		
		$alterar = "UPDATE tb_marcas SET status = 'inativo' WHERE id = '$id_alterar'";
		$alterar_exec = mysql_query($alterar); 
		if($alterar_exec) { 
		$alterar_produtos = "UPDATE tb_produtos SET status = 'inativo' WHERE id_marca = '$id_alterar'";
		$alterar_produto_exec = mysql_query($alterar); 
		$sucesso = "65"; 
		} else { $erro = "65"; }		
		}
		...
		$busca_visual = mysql_query("SELECT * FROM tb_marcas $where_busca $ordem_busca LIMIT $ini_busca, $fim_busca");
		$busca_completa = mysql_query("SELECT * FROM tb_marcas $where_busca");
